 ...
 * 
 * â€” minor fixes in formgen and modal
 *
...
; (function ($) {
    ...
    var MY = {
        ...
        msg: {
            "": { en: "Invalid input", ru: "ÐÐµÐ²ÐµÑ€Ð½Ð¾Ðµ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð¸Ðµ" },

            formError: { en: "Form error", ru: "ÐžÑˆÐ¸Ð±ÐºÐ° Ñ„Ð¾Ñ€Ð¼Ñ‹" },
            initFailed: {
                en: '<p class="my-error">Form init failed</p>',
                ru: '<p class="my-error">ÐžÑˆÐ¸Ð±ÐºÐ° Ð¸Ð½Ð¸Ñ†Ð¸Ð°Ð»Ð¸Ð·Ð°Ñ†Ð¸Ð¸ Ñ„Ð¾Ñ€Ð¼Ñ‹</p>'
            },

            badInput: { en: "Invalid input", ru: "ÐÐµÐ²ÐµÑ€Ð½Ð¾Ðµ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð¸Ðµ" },
            patternMismatch: { en: "Pattern mismatch", ru: "ÐÐµ ÑÐ¾Ð¾Ñ‚Ð²ÐµÑ‚ÑÑ‚Ð²ÑƒÐµÑ‚ ÑˆÐ°Ð±Ð»Ð¾Ð½Ñƒ" },
            rangeOverflow: { en: "Over maximum", ru: "Ð‘Ð¾Ð»ÑŒÑˆÐµ Ð¼Ð°ÐºÑÐ¸Ð¼ÑƒÐ¼Ð°" },
            rangeUnderflow: { en: "Under minimum", ru: "ÐœÐµÐ½ÑŒÑˆÐµ Ð¼Ð¸Ð½Ð¸Ð¼ÑƒÐ¼Ð°" },
            stepMismatch: { en: "Step mismatch", ru: "ÐÐµ ÐºÑ€Ð°Ñ‚Ð½Ð¾ ÑˆÐ°Ð³Ñƒ" },
            tooLong: { en: "Too long", ru: "Ð¡Ð»Ð¸ÑˆÐºÐ¾Ð¼ Ð´Ð»Ð¸Ð½Ð½Ð¾" },
            typeMismatch: { en: "Invalid type", ru: "ÐÐµÐ²ÐµÑ€Ð½Ñ‹Ð¹ Ñ‚Ð¸Ð¿" },
            valueMissing: { en: "Required", ru: "ÐžÐ±ÑÐ·Ð°Ñ‚ÐµÐ»ÑŒÐ½Ð¾Ðµ Ð¿Ð¾Ð»Ðµ" }
        },


        // Different controlsâ€™ events to watch for
        // ########################################################
    ...
    var f = ({
        ...
        "base64": function (s0, decode, prefix0) {
            ...
            // If prefix is a string, itâ€™s assumed mime,
        ...
        "css2json": function (css) {
            var acc = "", res = (
                (css + "")
                    ...
                    .replace(/@charset[^;]+;/gim, '')
                    .replace(/[\n\t\s]+/g, ' ')
                    .replace(/\s*@media[^{]*\{/g, function (e) { return e.to(-1).trim() + "á´¥" })
                    .replace(/\}/g, '}á´¥')
                    .replace(/^\n+/g, "").replace(/[\n\s]+$/g, "")
                    .split('á´¥')
                    .compact(true)
    ...
    function _overlap(a, b) {
        ...
        // non-obj properties are replaced, obj â€“ merged plain
    ...
    function _localCache(A1, A2) {
        // ( no args ) â€“ returns all forms obj container
        // ({manifest},  {container}) â€“ caches form in container, id must be defined, return form or null
        // ({manifest}) â€“ caches form in local container, id must be defined
        // ("form.Id", "exist") â€“ true/false
        // ("form.Id", {container}) â€“ get from container
        // ("form.Id") â€“ get from local cache
        var ref, obj;
        if (isS(A1)) {
            ref = A1;
    ...
    function _putmanifest(obj0, root0) {
        ...
        if (!(isO(root) && isO(obj) && isO(obj.ui) && isS(obj.id)))
            return "Canâ€™t save manifest into cache, invalid arguments.";
        ...
        try { obj = Clone(obj0, true); }
            ...
            return "Canâ€™t save circular-referencing object into cache.";
        }
        ...
        if (prev = _mask(root, id)) {
            if (prev.params && prev.params.protect) return "Canâ€™t save manifest into cache over protected one.";
            else {
                // Remove prev version
        ...
        if (ie8) res["_self"] = root._src[id];
            ...
            set: function () { throw "Canâ€™t change manifest cache entry directly."; },
            enumerable: false,
            configurable: true
        });
            ...
            for (i = 0; i < r.length; i++) {
                ...
                if (isS(line)) checks[line] = _exist(line);
                    ...
                    for (j in line) {
                        row = null;
                        if (line[j] === true) {
                            // global, we canâ€™t load it, just check presence
                            checks[j] = _exist(j);
                        }
                    ...
                    .then(function (data, row) {
                        if (rthis.test(row.ref)) {
                            if (data != null) {
                                ...
                                stop = true;
                                err.push('Invalid data for â€˜' + row.ref + 'â€™ resource.');
                            }
                        }
                        countdown();
                    }.fill(undefined, row))
                    .fail(function (e, row) {
                        err.push('Failed to load â€˜' + row.ref + 'â€™ resource.');
                        stop = true;
                        countdown();
                    }.fill(undefined, row));
            }
    ...
    function _run(A0, A1, A2) {
        ...
        if (isA(data.require)) {
            _require(data, p).then(_main)
                .fail(function (err) {
                    _makeup();
                    _f("Linker of the â€˜requireâ€™ property failed.", err);
                });
        }
        else _main();
        ...
        function _makeup() {
            ...
            // bind â€˜thisâ€™ to 1st level manifest functions
 ...
 *     1. jQuery image â€“ will raise modal with the image and text from title or data-text attributes
 *     2. HTML string â€“ will raise modal with html content
 *     3. Object of type
 *       {
 ...
 *         nose:"", left|right|top|bottom â€“ where to put nose
 ...
 *      opened and promise â€“ pending.
 *
 *     4. null, undefined or false â€“ close dialog and calls done(formErrors, data),
 *        if done return false or null promise is resoved with data,
 *        else modal stays open
 *     5. true (strict boolean) â€“ close dialog and calls done (null, null),
...
(function ($) {
    ...
    $.fn.modal = function (obj0, done0, width0) {
        ...
        // close btn
        if (m.close) {
            $cl = $(isS(m.close) ? m.close : '<div class="my-modal-close" title="Close">Ã—</div>')
